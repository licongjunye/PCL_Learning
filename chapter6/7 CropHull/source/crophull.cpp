...
int main(int argc, char** argv)
{
	...
	if (reader.read("../pig.pcd",*cloud) == -1)
	{
	...
	pcl::ConvexHull<pcl::PointXYZ> hull;	//hull用于计算点云的凸包
	hull.setInputCloud(boundingbox_ptr);
	hull.setDimension(2);
	std::vector<pcl::Vertices> polygons;	//	存储了凸包的多边形表示, 描述由点云中的点组成的多边形
	pcl::PointCloud<pcl::PointXYZ>::Ptr surface_hull (new pcl::PointCloud<pcl::PointXYZ>);	//surface_hull就是凸包,由凸包点组成
	...
	for_visualizer_v->addPolygon<pcl::PointXYZ>(surface_hull,0,.069*255,0.2*255,"backview_hull_polyline1",v1);	//点云可视化器中绘制多边形
	...
	for_visualizer_v->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR,0,0,0,"surface_hull");
	for_visualizer_v->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE,18,"surface_hull");
